...
[source,role="execute"]
----
bash ./support/machineset-generator.sh 3 workerocs 0 | oc create -f -
...
You will then need to select the size of your storage as shown below. Select `2 TiB` as the amount of effective storage for your cluster.
...
see all instances shown below as `Ready` Status in the Web Console as shown in the following diagram (some Status may be `Unknown`):
...
oc new-app -f ./support/ocslab_rails-app.yaml -p STORAGE_CLASS=ocs-storagecluster-ceph-rbd -p VOLUME_CAPACITY=5Gi
----
...
[source,role="execute"]
----
oc apply -f ./support/ocslab_cluster-monitoring-noinfra.yaml
----
.Sample output:
----
...
[source,role="execute"]
----
oc apply -f ./support/ocslab_cluster-monitoring-withinfra.yaml
----
.Sample output:
----
...
[source,role="execute"]
----
oc apply -f ./support/ocslab_obc-app-example.yaml
----
.Example output:
----
